...
describe BaiduCloudPush::Request do
  before(:each) do
    @api_key = 'api_key'
    @secret_key = 'secret_key'
    @client_enable_ssl = BaiduCloudPush::Client.new(@api_key,
                                               @secret_key,
                                               { use_ssl: true })
    @client = BaiduCloudPush::Client.new(@api_key,
                                    @secret_key)
    @method = :test
    @resource = 'channel'
  end
